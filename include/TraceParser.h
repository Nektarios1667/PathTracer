...
using traceFuncParseFunc = std::function < std::string(std::istringstream&)>;
...
public:
    ...
    traceFuncParseFunc parser;

    TraceFunc(std::string_view name, traceFuncParseFunc func) : name(name), parser(func) {}
};
...
private:
    ...
    static const std::vector<TraceFunc> traceFuncs;
public:
    static void injectVariables(std::string& text, ssMap localVariables);
    static void injectFunctions(std::string& text);
