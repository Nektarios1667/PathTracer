...
constexpr unsigned MAX_DEPTH   = RENDER_TYPE == RenderType::Light ? 5  : 1; // max bounced to prevent too much recursion
constexpr unsigned MIN_SAMPLES = RENDER_TYPE == RenderType::Light ? 8  : 1; // minimum adaptive samples
constexpr unsigned MAX_SAMPLES = RENDER_TYPE == RenderType::Light ? 32 : 1; // cutoff for adaptive sampling
